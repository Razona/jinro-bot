...
* **HTTPメソッド:** `POST`
* **パス:** `/game/setup`
* **説明:** ゲームのタイトルを設定し、そのタイトルをカテゴリ名としてDiscord上にゲーム用のカテゴリと基本的なチャンネル（GM用、投票用など、役職チャンネルを除く）を作成します。通常、ゲーム開始前に最初に呼び出されるAPIです。
* **トリガー:** スプレッドシートの [ゲーム準備開始] ボタン (想定)。
...
* **HTTPメソッド:** `POST`
* **パス:** `/role/list/add`
* **説明:** スプレッドシートで決定された配役情報をbotサーバーに送信し、Discord上での役職配布（役職チャンネル作成、招待、メンション）を依頼します。`/user-list/pull` の後に呼び出します。
